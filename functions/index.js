...
const corsOptions = {
  origin: [
    ...
    "https://isp-itech.web.app", // Firebase Hosting
    "https://isp-itech.firebaseapp.com", // Firebase Hosting alternative
  ],
  credentials: true,
...
const FRONTEND_URL = "https://isp-itech.web.app";
  ...
  <head>
    ...
    <style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .header { 
        background: linear-gradient(135deg, #2563eb 0%, #06b6d4 50%, #0891b2 100%); 
        color: white; 
        padding: 40px 30px;
        text-align: center;
        position: relative;
      ...
      <div class="container">
        <div class="header">
          <div class="header-content">
            ...
            <p>Gestion Intelligente des Équipements - Ivoire Sûreté Portuaire</p>
          </div>
        </div>
        <div class="content">
          ${content}
        ...
        <div class="footer">
          <div class="footer-logo">iTech</div>
          <p>© 2024 iTech - Plateforme de gestion intelligente des équipements - Ivoire Sûreté Portuaire</p>
...
exports.sendActivationEmailHttp = functions.https.onRequest(
  async (req, res) => {
    ...
    try {
      ...
      const content = `
      <div class="welcome-message">
        <p><strong>🔧 Bienvenue sur iTech !</strong></p>
        <p>Votre compte a été créé avec succès sur notre plateforme de gestion intelligente des équipements.</p>
      </div>
      
      <h2>Activez votre compte pour commencer</h2>
      <p>Merci de rejoindre iTech, la plateforme dédiée à la gestion intelligente des équipements d'Ivoire Sûreté Portuaire. Pour accéder au système de suivi et de maintenance des équipements, vous devez confirmer votre adresse email.</p>
    ...
    `;
      ...
      const mailOptions = {
        from: `iTech <${EMAIL_CONFIG.user}>`,
        to: email,
        subject: "Activer votre compte iTech",
        html: getEmailTemplate(
          content,
          "Activation de votre compte",
          "activation"
      ...
      console.error(
        "Erreur lors de l'envoi de l'email d'activation HTTP:",
        error
      );
      res.status(500).json({
...
exports.sendPasswordResetEmail = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const content = `
      ...
      <p>Vous avez demandé la réinitialisation de votre mot de passe pour votre compte iTech. Cliquez sur le bouton ci-dessous pour définir un nouveau mot de passe et accéder à nouveau à la plateforme de gestion des équipements.</p>
    ...
    const mailOptions = {
      from: `Nexion <${EMAIL_CONFIG.user}>`,
      to: email,
      subject: "Réinitialisation de mot de passe iTech",
      html: getEmailTemplate(
        content,
  ...
  async (req, res) => {
    ...
    try {
      ...
      <div class="welcome-message">
        ...
        <p>Votre mot de passe a été modifié le ${new Date().toLocaleDateString(
          "fr-FR"
        )} à ${new Date().toLocaleTimeString("fr-FR")}.</p>
      </div>
      
      <h2>Votre compte est maintenant sécurisé</h2>
      <p>Votre nouveau mot de passe est maintenant actif. Vous pouvez vous connecter à votre compte iTech en utilisant vos nouvelles informations d'identification pour accéder à la gestion des équipements.</p>
      ...
      <div class="security-notice">
        <p><strong>🚨 Activité non autorisée ?</strong> Si vous n'avez pas effectué ce changement, contactez immédiatement notre support à ${
    ...
    `;
      ...
      const mailOptions = {
        from: `iTech <${EMAIL_CONFIG.user}>`,
        to: email,
        subject: "Confirmation de changement de mot de passe iTech",
        html: getEmailTemplate(
          content,
...
exports.createUserByAdmin = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "⚠️ Erreur lors de l'envoi de l'email (non critique):",
        emailError
      );
    }
